...
std::string WWXMLManager::getWWStringFromXML(const char* strXmlPath, const std::string &strKey, const std::string &strSection)
{
	...
	//加载XML文件
	WWXMLManager myXML;
	if (!myXML.loadXMLFile(strXmlPath))
	{
	...
	if (!node.isNULL())
	{
		...
		while (!childNode.isNULL())
		{
			if (strcmp(childNode.firstAttribute("name").value(), strKey.c_str()) == 0)
			{
				...
				//win32，模拟器上显示需要转换成gbk编码GBKToUTF8
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
				strResult = StringHelper::AnsiToUtf8(strResult.c_str());
#endif
				return strResult;
			}
