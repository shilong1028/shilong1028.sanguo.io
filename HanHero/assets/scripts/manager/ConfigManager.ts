...
export class BuilderInfo {
    ...
    constructor(name: string, level: number) {
        this.name = name;
        this.level = level;
        this.builderCfg = CfgMgr.getBuildConf(name);
    }
...
class CfgManager_class {
    ...
    //州配置表
    C_zhou_info: Map<number, st_zhou_info> = new Map<number, st_zhou_info>();
    SC_zhou_info = st_zhou_info;
    //武将配置表
    ...
    /**获取州配置数据 */
    getZhouConf(zhouId: string): st_zhou_info {
        let obj = this.C_zhou_info[zhouId];
        if (obj) {
            return obj.clone();
        } else {
            return null;
    ...
    getBuildConf(buildname: string): st_build_info {
        let obj = this.C_build_info[buildname];
        if (obj) {
            return obj.clone();
        } else {
            return null;
    ...
    getConfigInfo(name: string, path: string, data: any) {
        cc.resources.load(path, cc.TextAsset, (err, file:any) => {
            if (err) {
