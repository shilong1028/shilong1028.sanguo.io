...
@ccclass
@menu("Hall/builders/builderCommunity")
@executionOrder(0)
//脚本生命周期回调的执行优先级。小于 0 的脚本将优先执行，大于 0 的脚本将最后执行。该优先级只对 onLoad, onEnable, start, update 和 lateUpdate 有效，对 onDisable 和 onDestroy 无效。
@disallowMultiple
// 当本组件添加到节点上后，禁止同类型（含子类）的组件再添加到同一个节点

export default class BuilderCommunity extends cc.Component {
    ...
    initCommunity(builderInfo: BuilderInfo) {
        this.builderInfo = builderInfo
        this.attrLabel.string = Math.floor(CfgMgr.getBuildOutputByLv("community", this.builderInfo.level)) + "%";
    }
