...
export default class QiPanSc extends cc.Component {
    ...
    handleLaunchSkills(){
        ...
        let weapon: BallInfo = null;
        if(FightMgr.usedPlayerInfo.ballId > 0){
            weapon = new BallInfo(FightMgr.usedPlayerInfo.ballId);  
        }
    ...
    */
            ...
            if(this.bMultiLineMove == true){   //多行下移过程中
                ...
                if(this.bCreateNextRound == false){  //是否在检查完回合事件后进行新行砖块创建并下移
                    ...
                    if(this.nBricks.children.length == 0 || this.checkHasEnemyCurView() == false){   //当前视图内没有敌人了
                        ...
                        if(FightMgr.getFightScene().checkRandomFightSkillById(1) == true){    //一定概率使得某回合暂停下移
    ...
    showHpAction(harm: number, pos: cc.Vec2){
        ...
        let dir = cc.v2(0, 1).rotate(cc.misc.degreesToRadians(a));
