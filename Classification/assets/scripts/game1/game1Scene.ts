...
export default class Game1Scene extends cc.Component {
        ...
        if(this.bGameOver == false){
            ...
            if(this.beginTime >= 60.0){
                if(this.beginStep < 4){
                    this.beginTime = 0;
                    this.beginStep ++;
                    this.timeLabel.string = (4 - this.beginStep).toString();
                }else{
                    this.beginTime = 0;
    ...
    createOneRandomRubbish(){
        ...
        if(rubbishNode){
            ...
            rubbishNode.getComponent(Rubbish).initRubbish(parseInt(randIdStr), this);
        }
    }

    //垃圾落地
    handleRubbishEnd(rubbishType: number){
        if(this.fightRubbishType == rubbishType){
