...
export default class BagLayer extends cc.Component {
    ...
    openBagGridByType(gridType: number){
        this.bagListNode.active = true;
        this.initTableData(gridType);
    }

    onSkillBtn(){
        AudioMgr.playEffect("effect/ui_click");
        this.closeBagListView();
    ...
    //打开背包界面 //0装备小球，1饰品道具，2技能
    initTableData(gridType: number, bForceClear:boolean = false){
        if(bForceClear != true && this.curListType == gridType){
            return;
        }
        if(this.tableView){
        ...
        let shopArr = [];
        this.curListType = gridType; 

        if(gridType == 0){   //0装备小球
            shopArr = MyUserDataMgr.getBallListClone();
        }else if(gridType == 1){   //1饰品道具
            shopArr = MyUserDataMgr.getItemListClone();
        }

        this.tableView.openListCellSelEffect(false);   //是否开启Cell选中状态变换
        this.tableView.initTableView(shopArr.length, { array: shopArr, target: this, gridType: gridType}); 
    }
